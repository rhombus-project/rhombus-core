...
def createRefundTx(node, rawtx, script, lockTime, addrRefundFrom, addrRefundTo):
    p2sh = script_to_p2sh_rhom(script)
...
class AtomicSwapTest(RhombusTestFramework):
    ...
    def test_standardtx(self):
        ...
        p2sh_initiate = script_to_p2sh_rhom(scriptInitiate)
        ...
        p2sh_participate = script_to_p2sh_rhom(scriptParticipate)

        rawtx_p = nodes[1].createrawtransaction([], {p2sh_rhomicipate:amountB})
        ...
        p2sh_initiate = script_to_p2sh_rhom(scriptInitiate2)
