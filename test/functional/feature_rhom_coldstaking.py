...
class ColdStakingTest(RhombusTestFramework):
    ...
    def run_test(self):
        ...
        assert(ro[0] == keyhash_to_p2pkhwallet_rhom(hex_str_to_bytes(hashCoinstake)))
        ...
        assert(ro[0] == keyhash_to_p2pkhwallet_rhom(hex_str_to_bytes(hashSpend)))
        ...
        assert(ro[0] == keyhash_to_p2pkh_rhom(hex_str_to_bytes(hashCoinstake)))

        ro = nodes[0].deriverangekeys(1, 1, ekChange, 'false', 'false', 'false', 'true')
        assert(ro[0] == keyhash_to_p2pkh_rhom(hex_str_to_bytes(hashSpend)))
        ...
        txid = nodes[0].sendtypeto('rhom', 'blind', outputs, 'comment', 'comment-to', 4, 64, False, coincontrol)
