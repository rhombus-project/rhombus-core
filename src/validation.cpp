...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (::GetSerializeSize(tx, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) < (fRhombusMode ? MIN_STANDARD_TX_NONWITNESS_SIZE_PART : MIN_STANDARD_TX_NONWITNESS_SIZE))
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!fRhombusMode  // genesis coinbase is spendable when in Rhombus mode
        && fIsGenesisBlock) {
        if (!fJustCheck)
    ...
    bool fEnforceBIP30 = fRhombusMode || (!((pindex->nHeight==91842 && pindex->GetBlockHash() == uint256S("0x00000000000a4d0a398161ffc163c503763b1f4360639393e0e4c8e300e0caec")) ||
    ...
    if ((fRhombusMode && pindex->pprev) || pindex->nHeight >= chainparams.GetConsensus().CSVHeight) {
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - (fRhombusMode ? 0 : 1));
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, bool accept_block=false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    int nLockTimeFlags = 0;
    if ((fRhombusMode && pindexPrev) || nHeight >= consensusParams.CSVHeight) {
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        if (miSelf != m_block_index.end()) {
            ...
            if (fRhombusMode && !fRequested && !::ChainstateActive().IsInitialBlockDownload() && state.nodeId >= 0
    ...
    if (pindex == nullptr) {
        ...
        if (fRhombusMode && !::ChainstateActive().IsInitialBlockDownload() && state.nodeId >= 0) {
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (!pindex->pprev
            ...
            if (fRhombusMode && !IsInitialBlockDownload()) {
